...
void GetDS1307_2(char buff[]) {
	...
	sprintf(buff, "%01d-%02d/%02d/%02d\n", weekday_dec, day_dec, month_dec,
			year_dec);
}
...
int main(void) {
	...
	char buf[100];
	SH1106_Init();
	sprintf(buf, "%s", "CardID:");
	SH1106_GotoXY(12, 0); // goto 10, 10
	...
	while (1) {
		...
		if (TM_MFRC522_Check(CardID) == MI_OK) {
			SH1106_GotoXY(35, 20);
			GetDS1307_1(buff);
			HAL_UART_Transmit(&huart1, (const uint8_t*) buff, strlen(buff),
					1000);
